...
# v5.41: the module now defaults to the network's public bootstrap
# seeds when no peer config is given -- correct for distribution, wrong
...
check("30.1 пул developer_grants выделен и == 254,500 BIO (v5.41: половина ушла в server_rewards)",
      bc.net.emission.pools["developer_grants"] == bc.DEVELOPER_GRANTS_POOL_SIZE_V41,
...
# ── 31. Cryptographic agility foundation (scheme-aware address/verify) ──
print("[31] Криптографическая гибкость: обратная совместимость + готовность")

pk31, sk31 = bc.Dilithium.keygen()
...
addr_other_scheme = bc.pq.address(pk31, scheme_id="FUTURE01")
check("31.3 другая схема с тем же ключевым материалом => другой адрес "
      "(нет коллизий между схемами)",
      addr_other_scheme != addr_default,
      f"{addr_other_scheme} vs {addr_default}")

# 31.4 -- verify() без scheme_id продолжает работать как раньше (реальная
# подпись реальным ключом)
msg31 = "TRANSFER|BIO1TEST|1.0|nonce=1"
sig31 = bc.pq.sign(sk31, msg31)
check("31.4 pq.verify() без scheme_id -- настоящая подпись проходит",
      bc.pq.verify(pk31, msg31, sig31))

# 31.5 -- verify() с неизвестной схемой честно отклоняет, не падает
# с исключением и не проходит по ошибке
verify_unknown = bc.pq.verify(pk31, msg31, sig31, scheme_id="NONEXISTENT")
check("31.5 verify() с несуществующей схемой -- честно False, не исключение",
      verify_unknown is False)
...
check("31.6 sig_scheme нового кошелька по умолчанию == 'MLDSA44'",
      row31["sig_scheme"] == "MLDSA44", dict(row31) if row31 else None)

# Раздел 32 проверяется по ДЕЛЬТЕ, не по абсолютной валидности --
# более ранние разделы (1-6 и другие) намеренно заводят тестовые
# деньги напрямую через db.credit() в обход пулов (см. `injected` в
# разделе 9), из-за чего /verify закономерно не возвращает valid:true
# начиная с раздела 7 и далее -- это особенность тестового окружения,
# не баг biochain.py. Раздел 32 честно проверяет, что ИМЕННО его
# собственный код не добавляет НОВОГО расхождения поверх уже
# накопленного тестами 1-31, а не то, что /verify абсолютно чист.
def _current_excess():
    v = bc.verify()
    if v["valid"]:
        return 0.0
    import re
    m = re.search(r"diff: ([+-][\d,]+\.\d+)", v.get("message", ""))
    return float(m.group(1).replace(",", "")) if m else None

_excess_before_32 = _current_excess()

# ── 32. Server rewards (v5.42): governance-only payout ─────────────────
print("[32] server_rewards: разделение пула + выплата ТОЛЬКО через governance PROPOSAL/VOTE")

# 32.1 -- пул разделился ровно пополам, ничего не потеряно
check("32.1 server_rewards пул == 254,500 BIO",
      bc.net.emission.pools.get("server_rewards", 0) == bc.SERVER_REWARDS_POOL_SIZE,
      bc.sat_to_bio(bc.net.emission.pools.get("server_rewards", 0)))
already_granted32 = bc.db.conn.execute(
    "SELECT COALESCE(SUM(amount), 0) as total FROM developer_grants").fetchone()["total"]
check("32.2 developer_grants(текущий) + уже выданные гранты + server_rewards == исходные 509,000 BIO",
      bc.net.emission.pools["developer_grants"] + already_granted32 + bc.net.emission.pools["server_rewards"]
      == bc.DEVELOPER_GRANTS_POOL_SIZE,
      bc.sat_to_bio(bc.net.emission.pools["developer_grants"] + already_granted32 + bc.net.emission.pools["server_rewards"]))

# 32.3 -- старый impulse-путь CLAIM_SERVER_REWARD полностью отключён:
...
blk32_dead, err32_dead = bc.net.send(addr32, addr32, 0, pk32.hex(), sig32_dead.hex(), time.time(),
                                       kind="CLAIM_SERVER_REWARD", payload=json.dumps({"url": "https://x.example"}), nonce=1)
check("32.3 CLAIM_SERVER_REWARD impulse отклонён в Network.send (removed in v5.42)",
      blk32_dead is None and "no longer supported" in (err32_dead or ""), err32_dead)

# 32.4 -- /claim_server_reward endpoint отвечает честной ошибкой, не платит
resp32_old_endpoint = bc.claim_server_reward(bc.ClaimServerRewardBody(address=addr32, url="https://x.example"))
...
# обращения к каким-либо node-local таблицам вроде promoted_peers.
test_url32 = "https://test-server-32.example.com/api"
pool_before32 = bc.net.emission.pools["server_rewards"]
bal32_before = bc.db.get_balance(addr32)
ok32_gov, msg32_gov = bc.apply_governance_param(
    "server_reward",
    json.dumps({"address": addr32, "url": test_url32, "amount": 1000}),
    proposal_id=999032)
check("32.6 server_reward governance-выплата проходит", ok32_gov, msg32_gov)

bal32_after = bc.db.get_balance(addr32)
check("32.7 баланс адреса вырос ровно на 1,000 BIO",
      (bal32_after - bal32_before) == bc.bio_to_sat(1000), (bal32_before, bal32_after))
check("32.8 пул server_rewards уменьшился ровно на 1,000 BIO",
      bc.net.emission.pools["server_rewards"] == pool_before32 - bc.bio_to_sat(1000))

# 32.9 -- повторная governance-выплата на ТОТ ЖЕ url отклонена (идемпотентность
# по server_rewards_paid, заполняемой ТОЛЬКО этой функцией -- одинаково на
# любом узле, а не по локальному promoted_peers/server_claims)
ok32_dup, msg32_dup = bc.apply_governance_param(
    "server_reward", json.dumps({"address": addr32, "url": test_url32, "amount": 1000}),
    proposal_id=999033)
check("32.9 повторная governance-заявка на уже оплаченный url отклонена",
      not ok32_dup and "already" in msg32_dup, msg32_dup)

# 32.10 -- сумма выше потолка (SERVER_REWARD_MAX) отклонена
ok32_over, msg32_over = bc.apply_governance_param(
    "server_reward", json.dumps({"address": addr32, "url": "https://another.example/api", "amount": 999999}),
    proposal_id=999034)
check("32.10 сумма выше SERVER_REWARD_MAX отклонена governance-валидацией",
      not ok32_over, msg32_over)

# 32.11 -- /verify остаётся согласованным после governance-выплаты
_excess_after_32 = _current_excess()
check("32.11 раздел 32 не добавляет нового расхождения к supply invariant "
      "(дельта относительно состояния до раздела 32)",
      abs(_excess_after_32 - _excess_before_32) < 0.00000001,
      f"до={_excess_before_32}, после={_excess_after_32}")

# ── 33. Hash-based self-recognition (v5.43) ─────────────────────────────
print("[33] cryptographic self-recognition: INSTANCE_ID + genesis_hash, "
      "not just a URL string")

class _FakeResp:
    def __init__(self, data): self._data = data
    def json(self): return self._data

_orig_http_get = bc.http_requests.get if bc.HTTP_OK else None
_orig_http_ok  = bc.HTTP_OK

# 33.1 -- /peer/chain_info exposes both instance_id and genesis_hash
info33 = bc.peer_chain_info()
check("33.1 /peer/chain_info содержит instance_id",
      "instance_id" in info33 and info33["instance_id"] == bc.INSTANCE_ID, info33.get("instance_id"))
check("33.2 /peer/chain_info содержит genesis_hash == chain[0].hash",
      info33.get("genesis_hash") == (bc.net.chain[0].hash if bc.net.chain else ""), info33.get("genesis_hash"))

_pools_before_33 = dict(bc.net.emission.pools)   # this whole section is pure
# networking/self-recognition logic -- no impulse is ever applied, so pools
# must come out byte-for-byte identical at the end (33.9 checks this).

# 33.3 -- sync_with_peer(): a configured peer that answers with OUR OWN
# instance_id is recognized as this-same-server and removed from
# PEER_URLS, entirely by hash -- the URL string itself ("clone-url...")
# gives no hint of this; only the returned instance_id does.
bc.HTTP_OK = True
fake_self_url33 = "https://clone-url-that-looks-like-a-different-server.example/api"
bc.PEER_URLS.append(fake_self_url33)
def _get_self_echo(url, **kw):
    return _FakeResp({"chain_len": len(bc.net.chain),
                       "latest_hash": bc.net.chain[-1].hash if bc.net.chain else "0"*64,
                       "genesis_hash": bc.net.chain[0].hash if bc.net.chain else "",
                       "instance_id": bc.INSTANCE_ID})   # <- claims to be us
bc.http_requests.get = _get_self_echo
bc.sync_with_peer(fake_self_url33)
check("33.3 sync_with_peer распознал свой instance_id и удалил URL из PEER_URLS",
      fake_self_url33 not in bc.PEER_URLS, bc.PEER_URLS)

# 33.4 -- sync_with_peer(): a peer with a genuinely DIFFERENT genesis
# hash is refused (different network), without ever calling /peer/chain
_chain_fetch_called_33 = {"v": False}
foreign_genesis_url33 = "https://foreign-network-peer.example/api"
def _get_foreign_genesis(url, **kw):
    if url.endswith("/peer/chain"):
        _chain_fetch_called_33["v"] = True
    return _FakeResp({"chain_len": 999999,   # deliberately "ahead" -- if the
                       # genesis check didn't stop us, we'd try to fetch blocks
                       "latest_hash": "f" * 64,
                       "genesis_hash": "0" * 63 + "1",   # provably different
                       "instance_id": "definitely-not-our-instance-id"})
bc.http_requests.get = _get_foreign_genesis
bc.sync_with_peer(foreign_genesis_url33)
check("33.4 sync_with_peer отказывается синхронизироваться при несовпадающем genesis_hash",
      not _chain_fetch_called_33["v"])

# 33.5 -- try_promote_candidate(): a candidate that meets the
# confirmations threshold but answers with OUR OWN instance_id is
# refused promotion, no matter how many confirmations it collected.
url33_self_candidate = "https://another-alias-for-this-server.example/api"
bc.http_requests.get = _get_self_echo
promoted33_self = bc.try_promote_candidate(url33_self_candidate, confirmations=bc.promotion_threshold())
check("33.5 try_promote_candidate отказывает в промоушене собственного instance_id",
      promoted33_self is False and url33_self_candidate not in bc.PEER_URLS)

# 33.6 -- try_promote_candidate(): fail-OPEN on unreachability -- an
# unrelated, genuinely unreachable candidate is still promoted normally
# (this check must never turn confirmations-based promotion into an
# accidental liveness requirement).
def _get_unreachable(url, **kw):
    raise Exception("simulated network failure")
bc.http_requests.get = _get_unreachable
url33_unreachable = "https://unreachable-but-legit-candidate.example/api"
promoted33_unreachable = bc.try_promote_candidate(url33_unreachable, confirmations=bc.promotion_threshold())
check("33.6 try_promote_candidate НЕ блокирует промоушен при недоступности self-check (fail-open)",
      promoted33_unreachable is True and url33_unreachable in bc.PEER_URLS)
bc.PEER_URLS.remove(url33_unreachable)   # cleanup -- keep later sections' PEER_URLS state clean

# 33.7 -- /peer/announce: a URL that liveness-responds with OUR OWN
...
def _get_self_echo_then_fail(url, **kw):
    if url.endswith("/peer/chain_info"):
        return _get_self_echo(url, **kw)
    raise Exception("should never be reached -- self-check must short-circuit first")
bc.http_requests.get = _get_self_echo_then_fail
fastsync33 = bc.fast_sync_from_snapshot("https://self-bootstrap-alias.example/api")
check("33.8 fast_sync_from_snapshot распознаёт себя и не пытается читать checkpoints",
      fastsync33 is False)

# cleanup -- restore real http_requests.get / HTTP_OK exactly as they were
# before this section, so nothing here leaks into other test runs.
if _orig_http_get is not None:
    bc.http_requests.get = _orig_http_get
bc.HTTP_OK = _orig_http_ok

# 33.9 -- this entire section is pure networking/self-recognition logic:
# no impulse was ever applied, so every pool must be byte-for-byte
# unchanged -- the strongest possible guarantee that none of this
# touched the economy.
check("33.9 раздел 33 не тронул ни один пул (чисто сетевая логика, без impulse)",
      dict(bc.net.emission.pools) == _pools_before_33)
