...
// Native Android Share sheet (Bluetooth, WhatsApp, Drive, etc.) -- not the
// Web Bluetooth API, which is for BLE sensors, not file transfer.
...
// Wallets keyed by address in one map; ACTIVE_KEY tracks which to show
// on launch. Old single-wallet storage auto-migrates in, once.
...
// ── RECENT RECIPIENTS ─────────────────────────────────────────────────
// Per-wallet address book, local only (not chain data), keyed by sender
// so switching wallets doesn't leak contacts between them.
const RECENTS_MAX = 8;
...
// Deletes any saved wallet by address (unlike clearWallet(), which only
// handles the currently-active one). Falls back the same way if active.
...
// C — мутируемая палитра: Object.assign(C, THEMES[theme]) перекрашивает
// всё сразу, без правки сотен мест, где используется C.xxx.
const C = { ...THEME_DARK };
const font="'Courier New',monospace";
// sx-ключи — ФУНКЦИИ, не статичные объекты: статичный объект читает
// C.bg один раз навсегда, функция — заново при каждом рендере.
const sx={
